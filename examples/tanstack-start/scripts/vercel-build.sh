...
# by this TanStack Start example. The Vite plugin shells out to
# `napi build --release`, which needs cargo on PATH. CARGO_TARGET_DIR is
# redirected under node_modules/.cache so compiled crate artifacts also ride
# Vercel's build cache.
#
# The Nitro Vite plugin auto-selects its `vercel` preset when the VERCEL env
# var is present, writing the Build Output API directory at .vercel/output —
...
npm run build -w example-tanstack-start
