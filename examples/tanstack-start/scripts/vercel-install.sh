#!/usr/bin/env bash
# Vercel installCommand: install the whole npm workspace from the monorepo root
# (so the example resolves `vite-plugin-native-rust` locally), then ensure a
# usable Rust toolchain.
#
# This example is deployed with the Vercel project's Root Directory set to
# `examples/tanstack-start` and "Include files outside the Root Directory"
# enabled, so the command runs with cwd = examples/tanstack-start but the full
# monorepo is present. We resolve the repo root relative to this script.
#
