...
# (this example is a workspace member), then ensure a usable Rust toolchain.
#
# This example is deployed with the Vercel project's Root Directory set to
# `examples/remix-v3` and "Include files outside the Root Directory" enabled,
# so the command runs with cwd = examples/remix-v3 but the full monorepo is
...
# re-downloading. (Same pattern as examples/react-router.)
set -euo pipefail
