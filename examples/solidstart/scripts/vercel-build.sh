...
cd "$REPO_ROOT"
# Build the plugin package to dist/ first, then this example. `vinxi build`
# runs the three router builds plus Nitro's vercel preset, which writes the
# Build Output API v3 directory at examples/solidstart/.vercel/output.
npm run build -w vite-plugin-native-rust
npm run build -w example-solidstart
